...
import helpers_unoptimized as hp
...
def rmhmc(X,y,niter = 6000,burnin = 1000,nleapfrog = 6,nnewton = 1,alpha = 100):
    ...
    for it in range(niter):
        ...
        Xtheta,logitXtheta,prodlXtheta,G = hp.newG(X,new_theta,alpha)      
        I_Deriv,TraceI_Deriv = hp.partial_derivs(X,logitXtheta,prodlXtheta,G)        
        ...
        for j in np.arange(nsteps):
            ...
            Pro_momentum = hp.update_momentum(X,y,new_theta,Pro_momentum,TraceI_Deriv,I_Deriv,G,logitXtheta,alpha,step_size)

            # Update theta parameters (second step)
            new_theta = hp.update_parameter(X,new_theta,Pro_momentum,G,nnewton,alpha,step_size)
        ...
        Pro_H = -Pro_LL + Pro_D + (Pro_momentum.T.dot(la.inv(G).dot(Pro_momentum)))/2

        # Current value
        Cur_D = .5* (np.log(2) + D * np.log(np.pi) + np.log(la.det(O_G)))    
        Cur_H = - Cur_LL + Cur_D + (O_momentum.T.dot(la.inv(O_G).dot(O_momentum)))/2
        ...
        if it >= burnin:
            thetasamples[it-burnin,:] = theta.reshape(5,)
