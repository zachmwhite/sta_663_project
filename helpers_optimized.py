...
def newG(X,theta,alpha):
    ...
    return Xtheta,logitXtheta,prodlXtheta,G,I_G
...
def update_parameter(X,theta,Pro_momentum,I_G,nnewton,alpha,step_size):
    new_theta = theta
    OI_Gmomentum = I_G.dot(Pro_momentum)
    for FixedIter in range(nnewton):
        Xtheta,logitXtheta,prodlXtheta,G,I_G = newG(X,new_theta,alpha)
        I_Gmomentum = I_G.dot(Pro_momentum)
